CREATE OR REPLACE FUNCTION partman.autovacuum_reset(p_parent_schema text, p_parent_tablename text, p_source_schema text DEFAULT NULL, p_source_tablename text DEFAULT NULL) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    FOR v_row IN 
        SELECT partition_schemaname, partition_tablename FROM partman.show_partitions(p_parent_schema||'.'||p_parent_tablename, 'ASC')
    LOOP
