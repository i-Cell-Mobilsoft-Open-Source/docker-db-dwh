CREATE OR REPLACE FUNCTION partman.partition_data_id(p_parent_table text
...
SELECT general_type INTO v_control_type FROM partman.check_control_type(v_source_schemaname, v_source_tablename, v_control);
...
v_current_partition_name := partman.check_name_length(COALESCE(v_parent_tablename), v_min_partition_id::text, TRUE);
