CREATE OR REPLACE FUNCTION partman.apply_constraints(p_parent_table text, p_child_table text DEFAULT NULL, p_analyze boolean DEFAULT FALSE, p_job_id bigint DEFAULT NULL) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
...
IF p_child_table IS NULL THEN
    ...
    SELECT partition_tablename INTO v_last_partition FROM partman.show_partitions(v_parent_table, 'DESC') LIMIT 1;

    IF v_control_type = 'time' OR (v_control_type = 'id' AND v_epoch <> 'none') THEN
        SELECT child_start_time INTO v_last_partition_timestamp FROM partman.show_partition_info(v_parent_schema||'.'||v_last_partition, v_partition_interval, v_parent_table);
    ...
    ELSIF v_control_type = 'id' THEN
        SELECT child_start_id INTO v_last_partition_id FROM partman.show_partition_info(v_parent_schema||'.'||v_last_partition, v_partition_interval, v_parent_table);
    ...
    v_child_tablename := partman.check_name_length(v_parent_tablename, v_partition_suffix, TRUE);
