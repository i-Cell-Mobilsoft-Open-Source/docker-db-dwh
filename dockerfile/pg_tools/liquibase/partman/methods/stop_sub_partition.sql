CREATE OR REPLACE FUNCTION partman.stop_sub_partition(p_parent_table text, p_jobmon boolean DEFAULT true) RETURNS boolean
    LANGUAGE plpgsql 
    AS $$
DECLARE
...
DELETE FROM partman.part_config_sub WHERE sub_parent = p_parent_table;
