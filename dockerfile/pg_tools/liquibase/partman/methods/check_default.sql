CREATE OR REPLACE FUNCTION partman.check_default(p_exact_count boolean DEFAULT true) RETURNS SETOF partman.check_default_table
    LANGUAGE plpgsql STABLE
    SET search_path = partman,pg_temp
    AS $$
DECLARE
...
v_row                       record;
v_sql                       text;
v_trouble                   partman.check_default_table%rowtype;
...
FOR v_row IN 
    SELECT parent_table, partition_type FROM partman.part_config
LOOP
