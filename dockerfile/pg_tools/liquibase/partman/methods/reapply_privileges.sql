CREATE OR REPLACE FUNCTION partman.reapply_privileges(p_parent_table text) RETURNS void
    LANGUAGE plpgsql 
    AS $$
DECLARE
...
FOR v_row IN 
    SELECT partition_schemaname, partition_tablename FROM partman.show_partitions(p_parent_table, 'ASC', p_include_default := true)
LOOP
