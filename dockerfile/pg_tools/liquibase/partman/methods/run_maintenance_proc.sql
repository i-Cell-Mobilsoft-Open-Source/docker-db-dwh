CREATE OR REPLACE PROCEDURE partman.run_maintenance_proc(p_wait int DEFAULT 0, p_analyze boolean DEFAULT NULL, p_jobmon boolean DEFAULT true)
    LANGUAGE plpgsql
    AS $$
DECLARE
 ...
 */
    v_sql := format('SELECT %I.run_maintenance(%L, p_jobmon := %L',
        'partman', v_row.parent_table, p_jobmon);
