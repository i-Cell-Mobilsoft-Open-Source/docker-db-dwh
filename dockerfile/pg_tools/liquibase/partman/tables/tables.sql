CREATE TABLE IF NOT EXISTS partman.logs(
  id UUID NOT NULL ,
...
CREATE INDEX IF NOT EXISTS part_config_type_idx ON partman.part_config (partition_type);
...
CREATE TABLE IF NOT EXISTS partman.part_config_sub (
    ...
    , CONSTRAINT part_config_sub_sub_parent_fkey FOREIGN KEY (sub_parent) REFERENCES partman.part_config (parent_table) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED
...
ALTER TABLE partman.part_config DROP CONSTRAINT IF EXISTS control_constraint_col_chk;
ALTER TABLE partman.part_config ADD CONSTRAINT control_constraint_col_chk CHECK ((constraint_cols @> ARRAY[control]) <> true);
ALTER TABLE partman.part_config_sub DROP CONSTRAINT IF EXISTS control_constraint_col_chk;
ALTER TABLE partman.part_config_sub ADD CONSTRAINT control_constraint_col_chk CHECK ((sub_constraint_cols @> ARRAY[sub_control]) <> true);

ALTER TABLE partman.part_config DROP CONSTRAINT IF EXISTS retention_schema_not_empty_chk;
ALTER TABLE partman.part_config ADD CONSTRAINT retention_schema_not_empty_chk CHECK (retention_schema <> '');
ALTER TABLE partman.part_config_sub DROP CONSTRAINT IF EXISTS retention_schema_not_empty_chk;
ALTER TABLE partman.part_config_sub ADD CONSTRAINT retention_schema_not_empty_chk CHECK (sub_retention_schema <> '');
...
CREATE INDEX IF NOT EXISTS custom_time_partitions_partition_range_idx ON partman.custom_time_partitions USING gist (partition_range);
 ...
 */
CREATE OR REPLACE VIEW partman.table_privs AS
 ...
 */
CREATE OR REPLACE FUNCTION partman.check_automatic_maintenance_value (p_automatic_maintenance text) RETURNS boolean
...
ALTER TABLE partman.part_config DROP CONSTRAINT IF EXISTS part_config_automatic_maintenance_check;
ALTER TABLE partman.part_config
ADD CONSTRAINT part_config_automatic_maintenance_check
CHECK (partman.check_automatic_maintenance_value(automatic_maintenance));

ALTER TABLE partman.part_config_sub DROP CONSTRAINT IF EXISTS part_config_sub_automatic_maintenance_check;
ALTER TABLE partman.part_config_sub ADD CONSTRAINT part_config_sub_automatic_maintenance_check
CHECK (partman.check_automatic_maintenance_value(sub_automatic_maintenance));


/*
 * Check function for config table epoch types
 */
CREATE OR REPLACE FUNCTION partman.check_epoch_type (p_type text) RETURNS boolean
...
ALTER TABLE partman.part_config DROP CONSTRAINT IF EXISTS part_config_epoch_check;
ALTER TABLE partman.part_config
ADD CONSTRAINT part_config_epoch_check 
CHECK (partman.check_epoch_type(epoch));

ALTER TABLE partman.part_config_sub DROP CONSTRAINT IF EXISTS part_config_sub_epoch_check;
ALTER TABLE partman.part_config_sub ADD CONSTRAINT part_config_sub_epoch_check 
CHECK (partman.check_epoch_type(sub_epoch));


/*
 * Check for valid config table partition types
 */
CREATE OR REPLACE FUNCTION partman.check_partition_type (p_type text) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER
    SET search_path TO pg_catalog, pg_temp
    AS $$
DECLARE
...
ALTER TABLE partman.part_config DROP CONSTRAINT IF EXISTS part_config_type_check;
ALTER TABLE partman.part_config ADD CONSTRAINT part_config_type_check 
CHECK (partman.check_partition_type(partition_type));

ALTER TABLE partman.part_config_sub DROP CONSTRAINT IF EXISTS part_config_sub_type_check;
ALTER TABLE partman.part_config_sub ADD CONSTRAINT part_config_sub_type_check
CHECK (partman.check_partition_type(sub_partition_type));
